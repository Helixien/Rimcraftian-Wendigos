...
namespace Wendigos
{
	public class SitePartWorker_DownedWendigo : SitePartWorker
	{
		public override void Notify_GeneratedByQuestGen(SitePart part, Slate slate, List<Rule> outExtraDescriptionRules, Dictionary<string, string> outExtraDescriptionConstants)
		{
			...
			Log.Message("Downed wendigo: " + pawn, true);
		}
		...
		{
			if (Rand.Chance(0.6f) && Find.FactionManager.TryGetRandomNonColonyHumanlikeFaction_NewTemp(out Faction faction, tryMedievalOrBetter: true))
			{
				return faction;
			}
			return null;
