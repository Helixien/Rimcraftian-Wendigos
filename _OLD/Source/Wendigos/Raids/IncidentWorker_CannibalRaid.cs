...
namespace Wendigos
{
    public class IncidentWorker_CannibalRaid : IncidentWorker_RaidEnemy
    {
        protected override bool TryResolveRaidFaction(IncidentParms parms)
        {
            Faction faction = Find.FactionManager.FirstFactionOfDef(WendigosDefOf.RCW_TribeCannibal);
        ...
        {
            parms.raidArrivalMode = Rand.Chance(0.7f) ? PawnsArrivalModeDefOf.EdgeWalkIn : DefDatabase<PawnsArrivalModeDef>.GetNamed("EdgeWalkInGroups");
		}
		...
		protected LordJob MakeLordJob(IncidentParms parms, Map map, List<Pawn> pawns, int raidSeed)
		{
			...
			{
				return new LordJob_Cannibals_AssaultColony(parms.faction);
			}
